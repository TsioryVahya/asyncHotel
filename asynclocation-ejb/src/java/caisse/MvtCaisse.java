...
public class MvtCaisse extends ClassEtat{
    ...
    public ClassMAPTable createObject(String u, Connection c) throws Exception{
        ...
        System.out.println("Id OP "+this.getIdOp());
        if(this.getIdOp()!=null&&this.getIdOp().startsWith("RESA")){
            ReservationLib r = new ReservationLib();
            r.setId(this.getIdOp());
            ReservationLib[] rr = (ReservationLib[]) CGenUtil.rechercher(r,null,null,c,"");
            if(rr.length>0){
                if(this.getCredit()>rr[0].getResteAPayer()){
                    throw new Exception("Le montant saisie est superieur au montant reste a payer !");
                }
            }
        }
        if(this.getIdOrigine()!=null &&this.getIdOrigine().startsWith("VNT")){
            VenteLib vente = getVenteLib(c);
            if(this.getCredit()> vente.getMontantreste()){
                throw new Exception("Le montant saisi est superieur au montant reste a payer !");
            }
        }
    ...
    public ClassMAPTable validerObject(String u, Connection c) throws Exception{
        if(this.getIdOrigine()!=null &&this.getIdOrigine().startsWith("VNT")){
            VenteLib vente = getVenteLib(c);
            System.out.println("idOrigine  ============ "+vente.getIdReservation()+" this = "+this.getIdOrigine());
            if(vente.getIdOrigine()==null && vente.getIdReservation()!=null)
            {
                vente.setIdOrigine(vente.getIdReservation());
            }
            ...
            if(vente.getIdOrigine()!=null&&vente.getIdOrigine().startsWith("RESA"))
            {
                ...
                System.out.println("idOrigine  ============ "+vente.getIdOrigine());
